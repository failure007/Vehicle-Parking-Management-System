...
// For deleting    
    if($_GET['del']){
        $catid=$_GET['del'];
        $bookedSlotsQuery = mysqli_query($con, "SELECT * FROM tbbookedslots WHERE vehicle_cat = '$catid' and veh_status='in'");
        $bookedData = mysqli_fetch_assoc($bookedSlotsQuery);
        if(empty($bookedData)) {
            mysqli_query($con,"delete from tblcategory where ID ='$catid'");
            echo "<script>alert('Data Deleted');</script>";
            echo "<script>window.location.href='manage-category.php'</script>";
        } else {
            echo "<script>alert('Parking Slots are Booked with this vehicle category.');</script>";
        }
    }
